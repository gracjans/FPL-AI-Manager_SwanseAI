...
def download_newest_fpl_data(season: str):
    ...
    fs = fsspec.filesystem("github", org="vaastav", repo="Fantasy-Premier-League")
    fs.get(fs.ls(f"data/{season}/"), destination.as_posix(), recursive=True)


def predict_and_select(season: str, gameweek: int, model_path_from_root: str, scaler_path_from_root: str):
